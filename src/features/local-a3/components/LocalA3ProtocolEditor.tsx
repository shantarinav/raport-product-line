...
function ContextSummary({ protocol }: { protocol: LocalA3Protocol }) {
  ...
  return (
    <SectionCard title="Паспорт отклонения" description="Контекст зафиксирован из дашборда и нужен как основание разбора." Icon={ClipboardList}>
      <div className="grid gap-3">
        <div className="flex flex-wrap gap-2 text-xs font-semibold text-raport-muted">
          ...
          {protocol.source.fileName ? <span className="rounded-full border border-raport-border bg-raport-surface-soft px-2 py-1">{protocol.source.fileName}</span> : null}
        ...
        <div>
          ...
          {protocol.dashboardTitle ? <p className="mt-1 text-sm text-raport-muted">{protocol.dashboardTitle}</p> : null}
        </div>
    ...
    <div className="space-y-4">
      <div className="space-y-4">
          ...
          <SectionCard title="Причина" description="Зафиксируйте суть проблемы и рабочую гипотезу причины.">
            <div className="grid gap-3 md:grid-cols-2">
              <TextAreaField id="a3-problem" label={isCompact ? "Что не так" : "Что произошло"} value={protocol.form.problem} error={errors["form.problem"]} onChange={(value) => updateForm("problem", value)} />
              <TextAreaField id="a3-cause" label="Почему возникло" value={protocol.form.cause} error={errors["form.cause"]} onChange={(value) => updateForm("cause", value)} />
            </div>
          </SectionCard>
              ...
              {isCompact ? (
                ...
                <>
                  ...
                  {compactSaveStatus ? (
                    <span
                      className={cn(
                        "md:col-span-4 rounded-control border px-3 py-2 text-sm font-semibold",
                        saveError
                          ? "border-raport-danger-border bg-raport-danger-muted text-raport-danger"
                          : "border-raport-success-border bg-raport-success-muted text-raport-success",
                      )}
