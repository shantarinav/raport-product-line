...
async function main() {
  ...
  if (!input) {
    console.error(
      "Usage: npm run backend:raport-llm:evaluate -- --input labeled.csv [--output result.json] [--proxy http://127.0.0.1:8787/api/print/classify-personal] [--batch-size 10] [--request-timeout-ms 300000]",
    );
    process.exitCode = 1;
    return;
  }
