...
export class PrintLlmSqliteCache {
  constructor(filePath = resolve(".cache", "raport-llm-cache.sqlite"), options = {}) {
  ...
  open() {
    ...
    this.db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS a3_assist_responses (
  ...
  getA3Assist(snapshotHash) {
    const row = this.open().prepare("SELECT result_json FROM a3_assist_responses WHERE snapshot_hash = ?").get(snapshotHash);
    if (!row?.result_json) return null;
    try {
      return JSON.parse(row.result_json);
    } catch {
  ...
  putA3Assist(snapshotHash, value, meta) {
    this.open()
      .prepare(
        `
          INSERT INTO a3_assist_responses (snapshot_hash, schema_version, model, result_json, created_at, updated_at)
          VALUES (?, ?, ?, ?, datetime('now'), datetime('now'))
      ...
      .run(snapshotHash, String(meta.schemaVersion), String(meta.model), JSON.stringify(value));
  }

  countA3Assist() {
    const row = this.open().prepare("SELECT COUNT(*) AS count FROM a3_assist_responses").get();
    return Number(row?.count || 0);
  }
