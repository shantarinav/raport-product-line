...
describe("readRaportLlmConfig", () => {
  it("uses safe local defaults", () => {
    const config = readRaportLlmConfig({});

    expect(config.host).toBe("127.0.0.1");
    expect(config.port).toBe(8787);
    expect(config.model).toBe("qwen3:1.7b");
    expect(config.a3Model).toBe("qwen3:4b");
  ...
  it("parses network and security settings from env", () => {
    const config = readRaportLlmConfig({
      RAPORT_LLM_HOST: "0.0.0.0",
      RAPORT_LLM_PORT: "9999",
      RAPORT_LLM_ALLOWED_ORIGINS: "https://bi.ekb.ru, http://server:8080 ",
      RAPORT_LLM_API_KEY: "secret",
      RAPORT_LLM_REQUEST_BODY_LIMIT_BYTES: "2000000",
      RAPORT_LLM_HTTP_REQUEST_TIMEOUT_MS: "123000",
      RAPORT_LLM_HTTP_HEADERS_TIMEOUT_MS: "45000",
      RAPORT_LLM_HTTP_KEEP_ALIVE_TIMEOUT_MS: "7000",
      RAPORT_LLM_CONCURRENCY: "2",
      RAPORT_LLM_SQLITE_BUSY_TIMEOUT_MS: "8000",
    });
  ...
  it("falls back to safe numbers for invalid numeric env values", () => {
    const config = readRaportLlmConfig({
      RAPORT_LLM_PORT: "not-a-port",
      RAPORT_LLM_BATCH_SIZE: "0",
      RAPORT_LLM_REQUEST_BODY_LIMIT_BYTES: "-10",
      RAPORT_LLM_CONCURRENCY: "0",
      RAPORT_LLM_SQLITE_BUSY_TIMEOUT_MS: "abc",
    });
