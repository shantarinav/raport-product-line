...
function Get-PrintLlmBackendDir {
  return (Join-Path (Get-RaportRoot) "backend\raport-llm")
}
...
function Get-PrintLlmPidPath {
  return (Join-Path (Get-PrintLlmRuntimeDir) "raport-llm.pid")
}

function Get-PrintLlmStdoutPath {
  return (Join-Path (Get-PrintLlmRuntimeDir) "raport-llm.out.log")
}

function Get-PrintLlmStderrPath {
  return (Join-Path (Get-PrintLlmRuntimeDir) "raport-llm.err.log")
}
...
function Import-PrintLlmEnv {
  ...
  if (-not (Test-Path -LiteralPath $Path)) {
    ...
    throw "Файл настроек не найден: $Path. Сначала выполните deploy\raport-llm\init-env.ps1."
  }
...
function Split-PrintLlmAllowedOrigins {
  param([string]$Value = $env:RAPORT_LLM_ALLOWED_ORIGINS)
...
function Get-PrintLlmConfigFromEnv {
  $hostName = if ($env:RAPORT_LLM_HOST) { $env:RAPORT_LLM_HOST } else { "127.0.0.1" }
  $healthHost = if ($hostName -eq "0.0.0.0") { "127.0.0.1" } else { $hostName }
  $port = if ($env:RAPORT_LLM_PORT) { [int]$env:RAPORT_LLM_PORT } else { 8787 }
  $scheme = "http"
  ...
  return [pscustomobject]@{
    ...
    ApiKey = $env:RAPORT_LLM_API_KEY
    Model = if ($env:RAPORT_LLM_MODEL) { $env:RAPORT_LLM_MODEL } else { "qwen3:1.7b" }
...
function Get-PrintLlmAuthHeaders {
  $headers = @{}
  if ($env:RAPORT_LLM_API_KEY) {
    $headers["X-Raport-Backend-Key"] = $env:RAPORT_LLM_API_KEY
  }
  return $headers
}
