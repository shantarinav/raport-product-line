param(
  [string]$TaskName = "Raport LLM Backend",
  [switch]$RunNow
)
...
if (-not (Test-Path -LiteralPath $envPath)) {
  throw "Файл настроек не найден: $envPath. Сначала выполните npm run backend:raport-llm:init:local или init:lan."
}
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Description "Raport optional LLM backend" -Force | Out-Null
...
if ($RunNow) {
  ...
  Write-Host "Задача запущена. Через несколько секунд проверьте: npm run backend:raport-llm:status"
}
