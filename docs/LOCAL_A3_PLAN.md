...
## 3. Что переносится из Team-наработок

В worktree `C:\codex\raport-team-v01` найдены Team A3-наработки:

- `src/team/a3Protocol.ts`;
- `src/team/a3Protocol.test.ts`;
- `src/team/components/A3ProtocolForm.tsx`;
- `src/team/components/A3ProtocolBoard.tsx`;
- части страницы `src/team/pages/ReportViewPage.tsx`;
- план `docs/superpowers/plans/2026-06-24-team-a3-lite-protocol.md`.

Переиспользуем идеи:

- A3 как связка `отклонение -> причина -> контрмера/решение -> ответственный -> срок -> проверка эффекта`;
- статусная модель разбора;
- модель комментариев, привязанных к протоколу;
- карточная подача протокола;
- отдельная доменная логика построения A3-модели;
- визуальный принцип: компактная карточка протокола + форма записи + журнал истории.

Переиспользуем осторожно:

- отдельные паттерны JSX-компоновки из `A3ProtocolForm.tsx` и `A3ProtocolBoard.tsx`, но без API, auth, roles и Team-событий;
- идею `attention items` для подсветки просроченных сроков и отсутствующего исполнителя;
- тестовые сценарии доменной модели, переписав их под Local IndexedDB/Zod.

Не переносим дословно:

- русские тексты из Team A3-файлов, потому что часть файлов повреждена кодировкой;
- Team event-log как серверную модель;
- `TeamReportEvent`, `createTeamApiClient`, `AuthProvider`, `canEditReport`, роли и права.

## 4. Что не переносится

Запрещено переносить или добавлять в эту ветку:
